(function(){
  ...
  var Raamat = function(){
    ...
    //KГµik muutujad, mis on Гјldised ja muudetavad
    this.currentRoute = null; // hoiab meeles mis lehel hetkel on
    this.books = []; //kГµik tulevad siia sisse

    //panen rakenduse tГ¶Г¶le
    this.init();
  };

  //window.Raamat = Raamat;

  //kirjeldatud kГµik lehed
  Raamat.routes = {
    "home-view" : {
      render: function(){
  ...
  //kГµik funktsioonid siia sisse
  Raamat.prototype = {
    init: function(){
      //console.log('rakendus kГ¤ivitus');
      ...
      //saan kГ¤tte purgid localStorage kui on
    ...
    search: function(event){
      ...
      for(var i=0; i<list.length; i++){
        var li = list[i];
          //Гјhe list itemi sisu
          var stack = li.querySelector('.content').innerHTML.toLowerCase();
          //kas otsisГµna on olemas
    ...
    addNewClick: function(event){
      ...
      if(BookAuthor === '' || BookName === ''){
          ...
          document.querySelector('#show-feedback').innerHTML='KГµik read peavad tГ¤idetud olema';
      }else{
        ...
        document.querySelector('#show-feedback').innerHTML='Salvestamine Гµnnestus';
    ...
    routeChange: function(event){
      ...
      if(this.routes[this.currentRoute]){
        ...
        //kГ¤ivitan selle lehe jaoks ettenГ¤htud js
    ...
    updateMenu: function(){
      //kui menГјГјl on active-menu siis vГµtame Г¤ra
      document.querySelector('.active-menu').className=document.querySelector('.active-menu').className.replace(' active-menu', '');
      //kГ¤esolevale lehele lisan juurde
